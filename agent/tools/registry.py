...
class ToolRegistry:
    ...
    def tools(self, names: List[str], granted_permissions: Optional[List[str]] = None, actor: str = "unknown_agent") -> List[Any]:
        """
        把 subagent/profile 中的工具名列表转换为 DeepAgents 需要的 tool 对象列表。
...
def _lazy_tool(module_path: str, attr_name: str) -> Callable[[], Any]:
    ...
    tool_registry -> agent.tools.database_workflow_tool -> agent.llm -> 模型初始化。
    """
    def load_tool() -> Any:
...
tool_registry.register(ToolSpec(
    ...
    category="database",
    tool_factory=_lazy_tool("agent.tools.database_workflow_tool", "run_database_workflow"),
    risk="high",

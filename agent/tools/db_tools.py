...
这些工具直接面向 MySQL 读接口和爆品分析 SQL，是数据库 workflow 的底层能力。
它们带有 session 级循环熔断，防止绕过状态机时重复查 schema 或重复执行相同 SQL。
"""
...
from agent.platform.db import execute_read_sql_raw, get_table_schema_raw
...
def _record_db_tool_call(tool_name, args=None):
    # 工具 guard 现在是兜底保险：主要循环控制在 database_workflow_tool.py 的状态机节点里。
...
def get_table_schema(table_name)->str:
    """
    查询指定表的字段结构，不读取业务数据。
    用于让数据库助手快速确认字段名，避免为了看字段反复拉取 100 行样例。
    """

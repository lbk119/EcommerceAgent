...
这些函数是业务 Agent 工具箱的数据库节点：SQL 固定、只读、输入少，避免让 LLM 在日报、库存、
活动复盘这类高频任务里反复自由生成通用 SQL。上层业务 Agent 只负责编排受控子计划和生成表达层总结。
"""

from dataclasses import dataclass
import re

from agent.platform.db import current_data_scope_sql, execute_read_sql_raw
...
def parse_business_time_range(query: str) -> BusinessTimeRange:
    ...
    if any(keyword in compact_query for keyword in ("双十一", "11.11", "1111")):
        return BusinessTimeRange("custom", 30, custom_note="campaign_period_keyword", campaign_keyword="双十一")
...
def query_shop_profile() -> str:
    ...
    # 店铺画像来自平台表，仍使用当前身份上下文限制到当前租户和店铺。
...
def query_hot_products(time_range: BusinessTimeRange | None = None, limit: int = 5) -> str:
    ...
    这个查询是 DeepAgent 自由调库的确定性替代：一条 SQL 覆盖销售、流量、转化、价格、库存、活动和退款，
    避免 Agent 因为缺少终止信号而反复调用数据库助手。多租户/多店铺隔离由 api.context 中的身份
...
def _required_data_scope(alias: str) -> str:
    """返回业务数据工具必须附加的 tenant/shop SQL 过滤条件。"""

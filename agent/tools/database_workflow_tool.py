...
本文件用小型 LangGraph 状态机包裹数据库问答和写库审查流程，避免数据库 subagent 自由循环调用
schema/sql 工具。只读查询、候选写 SQL、sandbox 校验、人工审批和 merge 发布都在显式节点里推进。
"""
...
from agent.platform.db import execute_write_sql_raw
...
MAX_REPEATED_NODE_ARGS = int(os.getenv("DB_GRAPH_MAX_REPEATED_NODE_ARGS", "2"))
...
TABLE_HINTS = {
    ...
    "order_items": ("商品", "明细", "销售", "价格", "seller", "sku"),
...
def _record_node_guard(state, node_name, args=None):
    ...
    if _should_run_workflow_supervisor(state, node_name, guard):
        ...
        decision, reason = _evaluate_loop_with_supervisor_sync(supervisor_summary)
        state.update(guard.snapshot())
...
class _WorkflowLoopGuard:
    """Small deterministic guard for the database state machine."""

    def __init__(self, events=None, fingerprints=None, last_supervised_count=0):
        self.events = list(events or [])
        self.fingerprints = list(fingerprints or [])
        self.last_supervised_count = int(last_supervised_count or 0)

    def record_event(self, event_type: str, args: dict, description: str) -> str:
        fingerprint = json.dumps({"event_type": event_type, "args": args or {}}, ensure_ascii=True, sort_keys=True, default=str)
        self.events.append(description)
        self.fingerprints.append(fingerprint)
        repeated = sum(1 for item in self.fingerprints if item == fingerprint)
        if repeated > MAX_REPEATED_NODE_ARGS:
            return self.summary(f"database graph repeated the same node and arguments {repeated} times")
        return ""

    def should_supervise(self, max_rounds: int) -> bool:
        interval = max(3, int(max_rounds / 2))
        return len(self.events) - self.last_supervised_count >= interval

    def summary(self, reason: str) -> str:
        self.last_supervised_count = len(self.events)
        recent = "\n".join(f"- {item}" for item in self.events[-6:])
        return f"{reason}\nrecent database graph events:\n{recent}"

    def snapshot(self) -> dict:
        return {
            "loop_events": self.events[-20:],
            "loop_fingerprints": self.fingerprints[-20:],
            "loop_last_supervised_count": self.last_supervised_count,
        }


def _evaluate_loop_with_supervisor_sync(summary: str) -> tuple[str, str]:
    if os.getenv("DB_GRAPH_SUPERVISOR_ENABLED", "false").lower() not in {"1", "true", "yes", "on"}:
        return "continue", "database graph supervisor disabled"
    if "repeated the same node" in summary:
        return "abort", "deterministic loop guard detected repeated database graph state"
    return "continue", "no deterministic loop pattern detected"
...
1. 先给结论。
2. 说明关键数据证据。
3. 如果是商品爆品分析，覆盖销售、流量、转化、价格、库存、活动和退款风险；
4. 给出下一步运营建议；
5. 不要再要求继续查询。
""")
...
def _generate_sql(state):
    ...
    previous_errors = "\n".join(state.get("errors", [])[-3:]) or "?"
    previous_sql = "\n".join(state.get("sql_history", [])[-3:]) or "?"
    from agent.llm import get_standard_model

    response = get_standard_model().invoke(f"""
你是 MySQL 只读 SQL 生成器。只返回一条 SQL，不输出 Markdown，不要解释。

硬性规则：
- 只能生成 SELECT 或 WITH 开头的只读查询。
- 必须包含 LIMIT，除非最外层是聚合单行结果。
- 禁止 INSERT、UPDATE、DELETE、DROP、TRUNCATE、ALTER、CREATE。
- 如果上一条 SQL 失败，必须换一种查询方案，不能复用同一逻辑。
...
def _generate_write_sql(question):
    ...
    response = get_standard_model().invoke(f"""
你是 MySQL 数据库变更 SQL 生成器。只返回一条 SQL，不输出 Markdown，不要解释。

安全规则：
- 只能生成 INSERT、UPDATE、DELETE、CREATE、ALTER。
- UPDATE 或 DELETE 必须包含明确 WHERE 条件。
- 禁止 DROP、TRUNCATE。
- 不要生成多条语句。
...
def _validate_write_sql(sql):
    ...
    if re.search(r"\b(drop|truncate)\b", normalized_sql, re.IGNORECASE):
        return {"ok": False, "error": "候选 SQL 包含 DROP/TRUNCATE，高风险 DDL 已拒绝。"}
...
def _run_sandbox(write_sql):
    # 当前项目没有真实数据 fork 能力；这里先实现安全的“沙箱校验报告”。
    # 如果配置 DB_SANDBOX_DATABASE，则会在该库执行事务；否则只返回待审核计划，不在 sandbox 阶段碰任何库。
    # 测试阶段允许“无沙箱直连生产”：DB_ENABLE_PRODUCTION_MERGE=true 时，人工审核 continue 后会由 merge 节点写入 MYSQL_DATABASE。
    ...
    if not sandbox_database:
        if _is_production_merge_enabled():
            return {
                ...
                "message": "未配置 DB_SANDBOX_DATABASE；当前 DB_ENABLE_PRODUCTION_MERGE=true。sandbox 阶段只做安全校验，人工审核 continue 后将由 merge 节点直接写入 MYSQL_DATABASE 指向的生产库。",
            }
        return {
            "ok": True,
            "executed": False,
...
async def _request_write_approval(state):
    # 这里复用现有 task_runtime.interrupt/resume 链路：任务会暂停，前端展示审核摘要，人工点击继续/修订/终止后恢复。
...
async def run_database_workflow(question: str) -> str:
    ...
    内置轮次上限、SQL失败计数和历史SQL查重，避免数据库 Agent 自由循环。
    """
    reset_db_guard_state()

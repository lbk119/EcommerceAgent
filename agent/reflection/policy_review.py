"""策略建议审核与 deepagents-native policy override 写入。

Agent 完成任务后会生成 reflection；本模块把 reflection 转换为“待审核策略建议”，人工批准后才写入
prompt/policy_overrides.yml。deepagents-native runtime 会在构建 main system prompt 时读取这些已批准策略。
"""
...
def list_approved_policy_instructions() -> List[str]:
    """读取已批准、可注入 deepagents-native system prompt 的策略指令。"""
    ...
    if not isinstance(loaded, dict):
        return []
    main_agent = loaded.get("main_agent") or {}
    items = main_agent.get("system_prompt_append") or []
...
def _append_policy_override(proposal: dict) -> None:
    ...
    agent.subagent.runtime 会在下次 reload 时把这些 instruction 合并到主 Agent system prompt。
    """

...
from agent.trace.reader import list_task_traces
...
def diagnose_task(task_id: str) -> dict[str, Any]:
    ...
    if not events:
        return {"taskId": task_id, "found": False, "recommendations": ["未找到 trace 事件，请确认 taskId 是否正确、trace 写入是否开启。"]}
...
def _recommendations(events: list[dict[str, Any]], model_calls: int, tool_calls: int, subagent_calls: int, critic_events: list[dict[str, Any]]) -> list[str]:
    ...
    event_types = [str(event.get("event_type") or "") for event in events]
    metadata_list = [event.get("metadata") or {} for event in events]
    if any(metadata.get("workflow_name") for metadata in metadata_list) and any("deepagents" in event_type.lower() for event_type in event_types):
        recommendations.append("该任务已进入 DeepAgents 主链路，请优先检查 subagent/tool 耗时和 guard 预算。")
    if model_calls > 2:

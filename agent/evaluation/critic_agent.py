"""Critic Agent 的窄口复核入口。

Critic 不替代主 Agent，也不会审查所有输出。它只针对高价值或高风险经营任务做结构化质量校验，
例如经营日报、SQL/写库、活动复盘、库存补货、退款异常分析等。

本阶段 Critic 的职责是发现明显缺项、识别证据不足和给出修复指令；它不会自动无限重试，
也不会绕过主运行时直接改写工具结果。
"""
...
def critic_required_for_task(query: str) -> bool:
    """兼容旧调用方：实际策略已迁移到 agent.evaluation.critic_policy。"""
...
def _build_critic_prompt(task_query: str, final_result: str) -> str:
    """构造有长度边界的 Critic prompt。"""
    return f"""
你是电商运营 Agent Critic，只负责质量校验，不要重写答案。
...
def _parse_critic_response(response: Any) -> CriticResult:
    """解析 Critic JSON 响应，兼容 Markdown fenced code block。"""

"""Central LLM backend / model router.

The commercial runtime intentionally exposes only two model tiers:

- fast tier: AI chat, standard workflows, reducers, critic/supervisor checks.
- deep tier: explicitly deep Agent tasks only.

Deployments should configure only LLM_FAST_MODEL and LLM_DEEP_MODEL.
"""
...
class ModelProfile:
    """Resolved immutable configuration for one semantic model profile."""
...
class LLMTracingCallback(BaseCallbackHandler):
    """Write LangChain model-call telemetry into the unified trace stream."""
    ...
    def _latency(self, run_key: str) -> float:
        """Return elapsed milliseconds for one model call."""
...
class LLMRouter:
    """Process-level model router with lazy model construction."""
    ...
    def get(self, profile_name: str = "standard_model"):
        """Return a lazily-built ChatModel by semantic profile name."""
    ...
    def _build_model(self, profile: ModelProfile):
        """Build a raw LangChain ChatModel for DeepAgents compatibility."""
        kwargs: Dict[str, Any] = {
            "model": profile.model,
...
def _extract_token_usage(response: Any) -> Dict[str, Optional[int]]:
    """Best-effort token usage extraction across OpenAI-compatible providers."""
...
def _estimate_prompt_chars(prompt_payload: Any) -> int:
    """Best-effort prompt size estimate for trace metadata."""
    try:

...
function stageDisplay(eventType: string, rawStage: string, data: Record<string, unknown>, event: AgentProgressEvent) {
  ...
  if (eventType.startsWith('evaluation')) return { stage: 'evaluation', group: '质量检查', title: eventType.endsWith('skipped') ? '已跳过完整质量检查' : '质量检查完成', detail: eventType.endsWith('skipped') ? 'AI Chat 使用轻量模式，完整 Evaluation 留给深度任务。' : '已完成结果校验。', status: eventType.endsWith('skipped') ? 'skipped' : 'completed', importance: 'normal' }
